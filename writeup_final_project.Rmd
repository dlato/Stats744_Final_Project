...
Since synonomous substitutions do not cause a change in an amino acid sequence, we expect most genes to have a synonomous rate (dS) larger than non-synonomoys rate (dN). 
...
An $\omega$ value below one means that the genes are likely neutral or under negative selection, meaning that mutations having deleterious impact on the organism will be removed over time.
...
As metioned previously, genes that are highly conserved and essential to the function of the organism are usually found near the origin of replication and part of the core genome.
Genes that are less conserved and essential are usually found near the terminus of replication and are used in local environmental adaptation, these are a part of the accessory genome.
...
(the part of the genome containing functionally important genes), is located about 3Mbp from the origin of replication, while the accessory genome is located approximately 2Mbp from the terminus or replication.
...
#sinoC
data11 <- read.delim("./data/sinoC_gene_exp_bidirectional_data.txt", sep = "")
data11$tmp_pos = abs(data11$tmp_pos)
summary(data11)
data11$group<-cut(data11$tmp_pos, breaks = seq(880,2300000,50000)
                  ,labels = seq(1:45)
)
data11$avg = rep(0,3429)
for (i in 1:45) {
  y = data11$Exp[(data11$group == i)]
  data11$avg[(data11$group == i)] = mean(data11$Exp[(data11$group == i)])
}

(g2 <- ggplot(data = data11, mapping = aes(x=group,y=Exp)) 
  +geom_jitter(aes(color="#F3D9B1"),alpha=0.4, width = 0.2) 
  +geom_point(aes(y = avg, color = "#C29979")) 
  +geom_smooth(aes(x = as.numeric(group), y = avg,color = "#6494AA"),span=0.5) 
  +guides(color=FALSE)
...
# ecoli
data7 <- read.delim("./data/ecoli_gene_exp_bidirectional_data.txt",sep = "")
data7$tmp_pos = abs(data7$tmp_pos)
summary(data7)
data7$group<-cut(data7$tmp_pos, breaks = seq(480,3350000,75000)
                 ,labels = seq(1:44)
)
summary(data7$group)
data7$avg = rep(0,3364)
for (i in 1:44) {
  y = data7$Exp[(data7$group == i)]
  data7$avg[(data7$group == i)] = mean(data7$Exp[(data7$group == i)])
}

(g3 <- ggplot(data = data7, mapping = aes(x=group,y=Exp)) 
  +geom_jitter(aes(color="#F3D9B1"),alpha=0.4, width = 0.2) 
  +geom_point(aes(y = avg, color = "#C29979")) 
  +geom_smooth(aes(x = as.numeric(group), y = avg,color = "#6494AA"),span=0.5) 
  +guides(color=FALSE)
  ...
  +scale_color_manual(values = c("#6494AA","#C29979","#F3D9B1"))
  +ylab("log Expression (CPM)") 
  +xlab("Distance from the origin of replication (7.5Kbp)")
  + ggtitle(expression(paste(italic("E.coli"),)))
)

# bass
data8 <- read.delim("./data/bass_gene_exp_bidirectional_data.txt", sep = "")
data8$tmp_pos = abs(data8$tmp_pos)
summary(data8)
data8$group<-cut(data8$tmp_pos, breaks = seq(280,2350000,50000)
                 ,labels = seq(1:46)
)
summary(data8$group)
data8$avg = rep(0,4154)
for (i in 1:46) {
  y = data8$Exp[(data8$group == i)]
  data8$avg[(data8$group == i)] = mean(data8$Exp[(data8$group == i)])
}

(g4 <- ggplot(data = data8, mapping = aes(x=group,y=Exp)) 
  +geom_jitter(aes(color="#F3D9B1"),alpha=0.4, width = 0.2) 
  +geom_point(aes(y = avg, color = "#C29979")) 
  +geom_smooth(aes(x = as.numeric(group), y = avg,color = "#6494AA"),span=0.5) 
  +guides(color=FALSE)
...
# pSymA
data9 <- read.delim("./data/pSymA_gene_exp_bidirectional_data.txt", sep = "")
data9$tmp_pos = abs(data9$tmp_pos)
summary(data9)
data9$group<-cut(data9$tmp_pos, breaks = seq(513,980000,20000)
                 ,labels = seq(1:48)
)
summary(data9$group)
data9$avg = rep(0,1293)
for (i in 1:48) {
  y = data9$Exp[(data9$group == i)]
  data9$avg[(data9$group == i)] = mean(data9$Exp[(data9$group == i)])
}
(g5 <- ggplot(data = data9, mapping = aes(x=group,y=Exp)) 
  +geom_jitter(aes(color="#F3D9B1"),alpha=0.4, width = 0.2) 
  +geom_point(aes(y = avg, color = "#C29979")) 
  +geom_smooth(aes(x = as.numeric(group), y = avg,color = "#6494AA"),span=0.5) 
  +guides(color=FALSE)
  ...
  +scale_color_manual(values = c("#6494AA","#C29979","#F3D9B1"))
  +ylab("log Expression (CPM)") 
  +xlab("Distance from the origin of replication (2Kbp)")
  + ggtitle(expression(paste(italic("S.meliloti"), " pSymA")))
)

# pSymB
data10 <- read.delim("./data/pSymB_gene_exp_bidirectional_data.txt", sep = "")
data10$tmp_pos = abs(data10$tmp_pos)
summary(data10)
data10$group<-cut(data10$tmp_pos, breaks = seq(650,870000,20000)
                  ,labels = seq(1:43)
)
summary(data10$group)
data10$avg = rep(0,1570)
for (i in 1:43) {
  y = data10$Exp[(data10$group == i)]
  data10$avg[(data10$group == i)] = mean(data10$Exp[(data10$group == i)])
}

(g6 <- ggplot(data = data10, mapping = aes(x=group,y=Exp)) 
  +geom_jitter(aes(color="#F3D9B1"),alpha=0.4, width = 0.2) 
  +geom_point(aes(y = avg, color = "#C29979")) 
  +geom_smooth(aes(x = as.numeric(group), y = avg,color = "#6494AA"),span=0.5) 
  +guides(color=FALSE)
...
, as we move away from the origin, we barely see any significant changes in the expression values. Until we are around 35 (5Kbp) away from the origin, that is when expresion values starts to decrease. Which is expected, as that is where we expect most of our accessory genomes to be located.
...
We therefore chose to create multiple graphs for each bacterial replicon, or focus on only one particularaly interesting replicon.
...
Genes with dS = 0 means that there are no synonomous substitutions which grealty influences our predictions on what evolutionary trajectories and past events are acting on that gene.
...
We decided to facet based on bacteria because primarally we wanted to show differences within bacterial replicons rather than between all bacterial replicons.
...
We also removed any space between the facets to ensure there was less unecessary white space.

We experimented with many many versions of this graph, prominant versions can be found in the *Appendix* section.
Although $\omega$ is "unitless", dN and dS are not.
It was therefore necessary to have multiple lables for both sets of values.
...
Therefore, we believe that the figure presented in the main part of the document provides a compact, accurate, and prefereable depiction of the selection values.
...
This is partially because we are interested primaraly in the distribution of $\omega$ as distance from the origin of replication increases, and partially because the values of $\omega$ are generally larger than the values for the rates.
...
For this graphic we also had many itterations (see *Appendix* Figure A3 for the most prominant one).
...
It is particularaly easy to determine the locations of the core (~3Mbp from the origin of replication) and accessory genome (~ the last 2Mbp of the graph) in \strep
, which carry important predications about how selection might be acting upon those regions of the genome.
Showing the graphs in a horizontal manner gives a more indepth picture about the distribution of the selection values across the genome (relative to the origin of replication).
...
We allowed for confidence intervals, but we do not see much of it for most of the bacterias, except for pSymA and pSymB. 
...
rate_g2 <- (ggplot(strep_rates, aes(x=new_sections, y=value.mean, colour=class))
          ...
          + geom_smooth()
          #labels for the colours
          + annotate("text", x=5.5,y=0.015,label="dS", colour = "#A09ABC", size = 4)
          + annotate("text", x=5.6,y=0.0035,label="dN", colour = "#6494AA", size = 4)
          + scale_y_continuous(trans='log10',labels = function(x) ifelse(x == 0, "0", x), breaks=c(0.001,0.01,0.1, 1, 10))
          + scale_color_manual(values=colours_arr)
          #remove redundant xaxis
          + theme(
...
omeg_g2 <- (ggplot(strep_omeg, aes(x=new_sections, y=value.mean, colour=class))
           ...
           + geom_smooth()
           #labels for the colours
           + annotate("text", x=5.5,y=0.3,label="omega", parse=TRUE, colour = "#C29979", size = 8)
           + scale_y_continuous(trans='log10',labels = function(x) ifelse(x == 0, "0", x), breaks=c(0.001,0.01,0.1, 1, 10))
           + scale_color_manual(values=colours_arr)
           #omega = 1 reference line
           +  geom_hline(yintercept=1, linetype="dashed", color = "black", size=1)
