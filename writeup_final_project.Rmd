...
#choose colours
#colours_arr <- rep(c( "#8BC1C1","#928CAB","#BE6361"),num_of_plots)
...
There is a phylogenetic component to the calculation of dN, dS and $\omega$ and the programs are taking an average of all the substitutions that could have occured along any branch within the phylogentic tree, over the total number of sites in the gene.
...
This is dicussed in further detail in the Graphical Decisions section.
...
omeg_g <- (ggplot(strep_omeg, aes(x=new_sections, y=value.mean, colour=class))
           ...
           + geom_smooth()
           #labels for the colours
           #+ annotate("text", x=5.4,y=0.4,label="omega", parse=TRUE, colour = "#B0413E", size = 10)
...
The points on the above graph represent the mean expected number of substitutions per 10,000 bp (10Kbp) for both dN and dS.
This was calculated by averaging the dS and dN values respectively over each 10Kbp section of the genome. 
A non-linear trendline with confidence intervales was added to the plot to show an overall trend about how dN and dS respectively are chaning with distance from the origin of replication.
...
(the part of the genome containing functionally important genes), is located about 3 million base pairs (Mbp) from the origin of replication, while the accessory genome is located approximatly 2Mbp from the terminus or replication.
It is therefore plausable that core genes should have more synonomous substitutions, so the amino acid sequence is not altered, compared to the accessory genome.
It is conceivable that the accessory genome has more non-synonomous substitutions which could provide increased genetic diversity, assisting in for example producing new anitibiotics.
With the location of the accessory genome coenciding with areas where we see decreased dS and increased dN, we could infer that biologically this is what is happening.
...
This graph shows the mean $\omega$ calculated over each 10Kbp region of the \strep 
genome.
...
A non-linear trendline with confidence intervales was added to the plot to show an overall trend about how $\omega$ changes with distance from the origin of replication.
As metioned previously, we see that most secations of the genome have a mean $\omega$ value of less than 1, as expected.
...
Genes that have an $\omega$ value larger than 1 are thought to be under positive selection and confering some sort of benefit to the organism, and will therefore likely be maintained over time.
...
It is therefore conceivable that this area of the genome is mostly under positive selection and trying to "hold on" to mutations that are benificial to the organism.
...
and analizing the trends we see in that graph in depth.**
...
For \strep
, the core genome is located within aproximatly 3,000,000 bp from the origin of replication.
The accessory genome is generally located within the region approximatly 2,000,000 bp from the terminus of replication.
...
Trailing zeros were removed from the axis lables to again make it more readable.
...
We chose to apply transparancy and "jittering" (where applicable) to ensure that overlapping points were identifiable and to maximize the amount of points shown.
...
Since $\omega$ provides important information about the selective pressures acting on a gene, we wnated to choose a colour that was different from the dN and dS colours so it could stand out in the facet graph.
...
### Facet Graph
For this graph we wanted to be able to compare the dS, dN amd $\omega$ values across all bacterial replicons, so we decided on a facet graph.
This allows us to hilight overarching similarities or differences between the bacterial replicons.
...
### Individaul Selection Value Graphs
Since the range of values and units differ between the dN, dS and $\omega$ values, we decided to create two separate graphs.
One showing the mean dN and dS values over 10Kbp distances from the origin of replication, and another showing the mean $\omega$ values over 10Kbp distances from the origin of replication.
...
Since the $\omega$ values are so much higher than the dN or dS values, having all selection parameters (dN, dS and $\omega$) on the same graph would obscure any subtle changes in the paramaters with respect to distance from the origin of replication.
By using separate graphs, we are able to see these changes more clearly.

We chose to include a non-linear trendline to help show what overall patterns are happening as the selection parameters change with distance from the origin of replication.
This allows us to see peaks and valleys that we would not necessarally see with a simple linear model.
We allowed for confidence intervals on the line to help show the fit of the geom_smooth() line.

Again, we are primaraly interested in $\omega$ values that are larger than zero (see intro on positive selection).
We decided to add a reference line at 1 to help remind viewers about the differences between $\omega$ values that lie above and below this line.
