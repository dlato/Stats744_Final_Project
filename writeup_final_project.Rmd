...
This graph shows the mean selection value (dN, dS, or $omega$) calculated over each 10,000 base pair (bp) region of the \strep 
genome.
...
In Daniella's thesis, she fit models and analized how gene expression changes with distance from the origin of replication for all of the bacterial replicons.
To avoid repetition, we decided to graph the gene expression of only the \strep
genome versus the distance from the origin of replication and to be consistant with the selection analysis. 
...
From the trend line above, we see that in general Expression decreases as we move further away from the origin of replication. This decrease is definetly not linear, we observe some jumps through out the graph. When we are 3,600,000 bp away from the origin, we start to see more of a wave-like pattern.
...
We chose to show the data points as a strip plot with a boxplot and violin plot overlayed.
